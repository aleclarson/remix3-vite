...
// node_modules/.pnpm/@remix-run+dom@0.0.0-experimental-remix-jam.6/node_modules/@remix-run/dom/dist/lib/hydrated.js
function hydrated(href, component) {
  ...
  if (!finalExportName) {
    throw new Error('hydrated() requires either an export name in the href (e.g., "/js/module.js#ComponentName") or a named component function');
  }
...
function createStyleManager(layer = "rmx") {
  ...
  return { insert: insert2, remove: remove2, has };
}
...
function replace(curr, next, domParent, frame, scheduler, vParent, anchor) {
  anchor = anchor || findFirstDomAnchor(curr) || findNextSiblingDomAnchor(curr, vParent) || void 0;
...
function diffCssProp(curr, next, dom3) {
  ...
  if (prevClassName === className)
    return;
  if (prevClassName) {
    dom3.classList.remove(prevClassName);
    styleManager.remove(prevClassName);
  }
  if (css && className) {
    dom3.classList.add(className);
    styleManager.insert(className, css);
  }
}
function diffHostProps(curr, next, dom3) {
  let isSvg = dom3.namespaceURI === SVG_NS;
  if (next.css || curr.css) {
    diffCssProp(curr, next, dom3);
  }
  for (let name in curr) {
    if (isFrameworkProp(name))
      continue;
    if (!(name in next) || next[name] == null) {
      if (canUseProperty(dom3, name, isSvg)) {
        try {
          dom3[name] = "";
          continue;
        } catch {
      ...
      if (ns)
        dom3.removeAttributeNS(ns, attr);
      else
        dom3.removeAttribute(attr);
    }
  }
  for (let name in next) {
    ...
    if (prevValue !== nextValue) {
      ...
      if (attr === "style" && typeof nextValue === "object" && nextValue && !Array.isArray(nextValue)) {
        dom3.setAttribute("style", serializeStyleObject(nextValue));
        continue;
      }
      if (canUseProperty(dom3, name, isSvg)) {
        try {
          dom3[name] = nextValue == null ? "" : nextValue;
          continue;
        } catch {
        }
      }
      ...
      if (nextValue != null && (nextValue !== false || isAriaOrData)) {
        ...
        if (ns)
          dom3.setAttributeNS(ns, attr, attrValue);
        else
          dom3.setAttribute(attr, attrValue);
      } else {
        if (ns)
          dom3.removeAttributeNS(ns, attr);
        else
...
function canUseProperty(dom3, name, isSvg) {
  if (isSvg)
    return false;
...
function insert(node, domParent, frame, scheduler, vParent, anchor, cursor) {
  ...
  let doInsert = anchor ? (dom3) => domParent.insertBefore(dom3, anchor) : (dom3) => domParent.appendChild(dom3);
  if (isTextNode2(node)) {
    ...
    let dom3 = document.createTextNode(node._text);
    commitText(node, { _dom: dom3, _parent: vParent });
    doInsert(dom3);
    return cursor;
  }
  if (isHostNode(node)) {
    ...
    let dom3 = inSvg ? document.createElementNS(SVG_NS, node.type) : document.createElement(node.type);
    diffHostProps({}, node.props, dom3);
    diffChildren(null, node._children, dom3, frame, scheduler, node);
    commitHost(node, { _dom: dom3, _parent: vParent }, scheduler, domParent, frame);
    doInsert(dom3);
    return cursor;
  }
...
function renderComponent(handle, currContent, next, domParent, frame, scheduler, vParent, anchor, cursor) {
  let normalizedOn = next.props.on ? Array.isArray(next.props.on) ? next.props.on : [next.props.on] : void 0;
...
function findFirstDomAnchor(node) {
  ...
  if (isFragmentNode(node)) {
    for (let child of node._children) {
      let dom3 = findFirstDomAnchor(child);
      if (dom3)
        return dom3;
    }
  }
...
function findNextSiblingDomAnchor(curr, vParent) {
  ...
  for (let i = idx + 1; i < children.length; i++) {
    let dom3 = findFirstDomAnchor(children[i]);
    if (dom3)
      return dom3;
  }
  return null;
...
var defaultInit = {
  ...
  pendingHydrationRoots: /* @__PURE__ */ new Map(),
  src: "/",
...
// ../../node_modules/.pnpm/@remix-run+events@0.0.0-experimental-remix-jam.1/node_modules/@remix-run/events/dist/lib/events.js
var _debug2 = false;
function log2(...args) {
  if (_debug2) {
    console.log("DEBUG", ...args);
...
function events2(target, initialDescriptors) {
  let descriptors = [];
  let cleanups = [];
  let on = (nextDescriptors) => {
    if (!nextDescriptors) {
    ...
    if (descriptorsChanged2(descriptors, nextDescriptors)) {
      cleanupAll2(cleanups);
      cleanups = [];
      if (nextDescriptors.length > 0) {
        attachAllEvents2(target, nextDescriptors, cleanups);
      }
      descriptors = nextDescriptors;
    } else {
      updateHandlersInPlace2(descriptors, nextDescriptors);
    }
  };
  let cleanup = () => {
    cleanupAll2(cleanups);
...
function bind2(type, handler, options) {
  return { type, handler, options };
}
...
function createDispatcher2(target, type) {
  return (options, originalEvent) => {
    let customEvent = new CustomEvent(type, {
      bubbles: true,
      cancelable: true,
...
function prepareInteractions2(target, descriptors, cleanups) {
  let seenEventTypes = /* @__PURE__ */ new Set();
  for (let descriptor of descriptors) {
    if (seenEventTypes.has(descriptor.type)) {
      continue;
    ...
    seenEventTypes.add(descriptor.type);
    let dispatch = createDispatcher2(target, descriptor.type);
    let factoryResult = descriptor.factory({ dispatch, target }, descriptor.factoryOptions);
    if (factoryResult) {
      let factoryCleanups = Array.isArray(factoryResult) ? factoryResult : [factoryResult];
      cleanups.push(...factoryCleanups);
...
function attach2(target, eventType, descriptors, cleanups) {
  log2("attach", { target, eventType, descriptors });
  let preventedEvents = /* @__PURE__ */ new Set();
  for (let descriptor of descriptors) {
    let controller = new AbortController();
    let wrappedHandler = (event) => {
      controller.abort(new DOMException("Handler reentered", "EventReentry"));
      controller = new AbortController();
      log2("wrappedHandler", { target, eventType, event });
      if (preventedEvents.has(event)) {
        log2("prevented", { target, eventType, event });
        return;
      }
      let call = descriptor.handler(event, controller.signal);
      if (call instanceof Promise) {
...
function attachStandardEvents2(target, descriptors, cleanups) {
  let eventsByType = /* @__PURE__ */ new Map();
  for (let descriptor of descriptors) {
    if (!eventsByType.has(descriptor.type)) {
      eventsByType.set(descriptor.type, []);
  ...
  for (let [type, descriptors2] of eventsByType) {
    attach2(target, type, descriptors2, cleanups);
  }
}
function attachInteractions2(target, descriptors, cleanups) {
  let byType = /* @__PURE__ */ new Map();
  for (let descriptor of descriptors) {
    if (!byType.has(descriptor.type)) {
      byType.set(descriptor.type, []);
...
function attachAllEvents2(target, descriptors, cleanups) {
  let { custom, standard } = splitDescriptors2(descriptors);
  prepareInteractions2(target, custom, cleanups);
  attachInteractions2(target, custom, cleanups);
  attachStandardEvents2(target, standard, cleanups);
}
function splitDescriptors2(descriptors) {
  let custom = [];
  let standard = [];
  for (let descriptor of descriptors) {
    if (isInteractionDescriptor2(descriptor)) {
      custom.push(descriptor);
    } else {
      standard.push(descriptor);
    }
...
function isInteractionDescriptor2(descriptor) {
  return descriptor.isCustom === true;
}
function descriptorsChanged2(descriptors, nextDescriptors) {
  if (descriptors.length !== nextDescriptors.length) {
    return true;
  }
  for (let i = 0; i < descriptors.length; i++) {
    let current = descriptors[i];
    let next = nextDescriptors[i];
    if (current.type !== next.type || current.isCustom !== next.isCustom || !shallowEqual2(current.options, next.options) || !shallowEqual2(current.factoryOptions, next.factoryOptions)) {
      return true;
    }
  }
  return false;
}
function updateHandlersInPlace2(descriptors, nextDescriptors) {
  for (let i = 0; i < nextDescriptors.length; i++) {
    descriptors[i].handler = nextDescriptors[i].handler;
  }
}
function cleanupAll2(cleanups) {
  for (let cleanup of cleanups)
    cleanup();
}

// ../../node_modules/.pnpm/@remix-run+events@0.0.0-experimental-remix-jam.1/node_modules/@remix-run/events/dist/lib/targets.js
function createTargetProxy2() {
  return new Proxy(function targetFunction(type, handler, options) {
    return bind2(type, handler, options);
  }, {
    get(target, prop) {
      if (typeof prop === "string") {
        return function(handler, options) {
          return bind2(prop, handler, options);
        };
      }
      return target[prop];
    }
...
var dom2 = createTargetProxy2();
var xhr2 = createTargetProxy2();
var win2 = createTargetProxy2();
var doc2 = createTargetProxy2();
var ws2 = createTargetProxy2();

// ../../node_modules/.pnpm/@remix-run+events@0.0.0-experimental-remix-jam.1/node_modules/@remix-run/events/dist/lib/interactions.js
...
function createInteraction2(eventName, factory) {
...
// ../../node_modules/.pnpm/@remix-run+events@0.0.0-experimental-remix-jam.1/node_modules/@remix-run/events/dist/lib/event-type.js
function createEventType2(eventName) {
  let binder = (handler, options) => {
    return {
      type: eventName,
      handler,
...
// ../../node_modules/.pnpm/@remix-run+events@0.0.0-experimental-remix-jam.1/node_modules/@remix-run/events/dist/interactions/submit.js
var submit2 = (action) => {
  return dom2.submit((event) => {
...
export {
  ...
  bind2 as bind,
  connect,
  createEventType2 as createEventType,
  createFrame,
  createInteraction2 as createInteraction,
  createRangeRoot,
  createRoot,
  diffDom,
  disconnect,
